...
from tkinter import Button, Entry, Label, Scale, LEFT, CENTER, VERTICAL, END, DoubleVar, Menu, Toplevel
import time
...
class BFSmanager:
    ...
    def bfs(
        ...
        self.canvas.after(int(self.timing),
                          ...
                          fill='green'))
        tk.update()
        self.canvas.after(int(self.timing*2),
                          self.canvas.itemconfig(self.line,
                          fill='blue'))
        tk.update()
    ...
    def usingBFS(self):
        ...
        for x in range(len(matrix2)):
            graph[str(matrix2[x][0])] = (matrix2[x])[1:]
        visit = self.bfs(visited, graph, a,queue)
        self.bfsdestroyer()
        Output.delete(0, END)
        Output.insert(0, visit)
        print (visit)
...
if __name__ == "__main__":
    ...
    breath_manager= BFSmanager(canvas)
    ...
    btnDFS.place(x=1210, y=60, width=100, height=60)
    btnBFS = Button(text='BFS graph:', bg='pale green', font=('Roboto',
                    13), command=breath_manager.usingBFS)
    btnBFS.place(x=1210, y=0, width=100, height=60)
    btndestroy = \
